...
import 'package:provider/provider.dart'; //Make Widget -> 1. option+command+W 2.make dartfile 3.import ..
...
class Post extends StatelessWidget {
  ...
  Widget _postCaption() {
    return Padding(
      padding: const EdgeInsets.symmetric(
          horizontal: common_gap, vertical: common_xxs_gap), //padding
      child: Comment(
        showImage: false,
  ...
  Widget _postHeader() {
    return Row(
      children: <Widget>[
        Consumer(
          //데이터 받아올 수 있음 //여기서는 받아올 context 가 없으므로, provider.of 가 아니라 consumer 를 쓴다.
          builder: (context, data, child) {},
          child: Padding(
            //widget 간 겹치지 않게 padding
        ...
        //남은 나머지 자리를 차지하게 해주는 widget - Expanded
        IconButton(
          onPressed: null,
          icon: Icon(
            Icons.more_horiz,
  ...
  CachedNetworkImage _postImage() {
    //option,command+M - refactor
    return CachedNetworkImage(
        imageUrl: 'https://picsum.photos/id/$index/200/200',
        //사이에 index 값 끼워넣어서 random 이미지 생성
        placeholder: (BuildContext context, String url) {
          return MyProgressIndicator(
            containerSize: size.width,
          ); //여기서 크기 명시 안하면 mpi 에서 설정한 default 값인 60이 설정된다
        },
        //image 로딩되는 동안 보여즐 것
